...
export default function UserMenuModal({ currentUser, showUserMenu, setShowUserMenu }: Props) {
    return <>
      {showUserMenu && <div
        className="fixed inset-0 bg-linear-to-r from-primary-light/10 to-primary-light/20 backdrop-blur-xs z-20 transition-opacity duration-300"
        onClick={() => setShowUserMenu(false)}
      />}
      <div onClick={e => e.stopPropagation()} className="absolute right-4 top-full w-64 bg-tetiary rounded-xl shadow-lg border border-primary/20 py-2 z-30">
